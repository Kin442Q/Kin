...
export default function AnalyticsPage() {
  ...
  const profitTrend = yearTrend.map((p) => ({
    month: p.month,
    value: p.profit,
  }));
  ...
  const growthPct =
    lastTwo.length === 2 && lastTwo[0].profit !== 0
      ? ((lastTwo[1].profit - lastTwo[0].profit) /
          Math.abs(lastTwo[0].profit)) *
        100
      : 0;

  // ROI = чистая прибыль / расходы * 100%
  const roi =
    global.totalExpenses > 0 ? global.netProfit / global.totalExpenses : 0;

  // Cash flow (приток - отток) по 6 месяцам
  const cashFlow = yearTrend
    .slice(-6)
    .map((p) => ({
      month: p.month,
      type: "Приток",
      ...
      >
        <Alert
          ...
          message={
            <Space>
              ...
              <Tag color={profitGrowing ? "green" : "red"}>
                ...
                {isFinite(growthPct)
                  ? Math.max(-999, Math.min(999, growthPct)).toFixed(1)
                  : "0.0"}
                % за месяц)
              </Tag>
            </Space>
          ...
          description={
            <span>
              Доход:{" "}
              <Text strong>{formatMoneyCompact(global.totalIncome)}</Text> ·
              Расходы:{" "}
              <Text strong>{formatMoneyCompact(global.totalExpenses)}</Text> ·
              Чистая прибыль:{" "}
              <Text
                strong
                style={{ color: global.netProfit >= 0 ? "#10b981" : "#f43f5e" }}
              >
                {global.netProfit >= 0 ? "+" : ""}
          ...
          >
            <Card className="glass" bordered={false}>
              ...
              <div className="mt-3">
                ...
                />
                <Text type="secondary">
                  Привлечение детей (заполнено мест условно: 60 чел.)
                </Text>
                <Progress
                  percent={Math.max(
                    0,
                    Math.min(100, (children.length / 60) * 100),
                  )}
                ...
                columns={[
                  {
                    ...
                    render: (_, __, idx) => (
                      <Tag
                        color={
                          idx === 0 ? "gold" : idx === 1 ? "silver" : "default"
                        }
                      >
                        {idx + 1}
                      </Tag>
